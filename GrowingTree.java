package com.example.growingtree;
...
public class GrowingTree extends Application {
    ...
    // Maze class using the Growing Tree algorithm
    class Maze {
        ...
        public void generateMaze() {
            ...
            // Initialize the active list with the start cell
            List<Cell> activeList = new ArrayList<>();
            Cell start = getStartCell();
            grid[start.y][start.x] = path;
            activeList.add(start);

            while (!activeList.isEmpty()) {
                // Select a cell from the active list
                Cell current;
                if (activeList.size() == 1) {
                    current = activeList.get(0);
                } else {

                    current = activeList.get(activeList.size() - 1);
                }
